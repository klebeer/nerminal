...
fn agent_view_lifecycle_updates_input_mode() {
    App::test((), |mut app| async move {
        ...
        let _agentview_guard = FeatureFlag::AgentView.override_enabled(true);
...
fn root_cloud_mode_pane_sets_root_cloud_mode_context_key() {
    App::test((), |mut app| async move {
        ...
        let _agentview_guard = FeatureFlag::AgentView.override_enabled(true);
        let _cloudmode_guard = FeatureFlag::CloudMode.override_enabled(true);
...
fn test_scroll_position_doesnt_change_when_block_finished() {
    ...
    App::test((), |mut app| async move {
        ...
        // Scroll positions only fix in place when the input is pinned; the
        // waterfall layout this build defaults to keeps following the bottom.
        app.update(|ctx| {
            crate::settings::InputModeSettings::handle(ctx).update(ctx, |settings, ctx| {
                let _ = settings
                    .input_mode
                    .set_value(InputMode::PinnedToBottom, ctx);
            })
        });
