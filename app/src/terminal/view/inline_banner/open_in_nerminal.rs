...
use crate::terminal::view::open_in_nerminal::OpenablePath;
use crate::terminal::view::{InlineBannerId, TerminalAction};
use crate::util::openable_file_type::OpenableFileType;

#[derive(Clone, Copy, Debug)]
pub enum OpenInNerminalBannerAction {
    OpenFile,
    LearnMore,
    Close,
}

pub struct OpenInNerminalBannerState {
...
impl OpenInNerminalBannerState {
...
fn file_title_text(openable_path: &OpenablePath) -> String {
    match openable_path.file_type {
        OpenableFileType::Markdown => {
            "Did you know that Nerminal can directly display Markdown files?".to_string()
        }
            ...
            cfg_if::cfg_if! {
                if #[cfg(not(target_family = "wasm"))] {
                    ...
                    match language.as_ref().map(|language| language.display_name()) {
                        Some(display_name) => {
                            format!("Did you know that Nerminal can directly edit {display_name} files?")
                        }
                        None => "Did you know that Nerminal can directly edit code?".to_string(),
                    }
                } else {
                    // The `languages` crate is not available on WASM, so use a fallback message.
                    "Did you know that Nerminal can directly edit code?".to_string()
                }
            }
        }
    }
...
pub fn render_open_in_nerminal_banner(
    state: &OpenInNerminalBannerState,
    view_id: EntityId,
    appearance: &Appearance,
) -> Box<dyn Element> {
    let button_text = match state.target.file_type {
        OpenableFileType::Markdown => "View in Nerminal",
        OpenableFileType::Code | OpenableFileType::Text => "Edit in Nerminal",
    };
    ...
    let open_button = InlineBannerTextButton {
        ...
        button_state: InlineBannerButtonState {
            on_click_event: TerminalAction::OpenInNerminalBanner(
        ...
        position_id: Some(format!("open_in_nerminal_banner_button_{view_id}")),
    ...
    let close_button = InlineBannerCloseButton(InlineBannerButtonState {
        on_click_event: TerminalAction::OpenInNerminalBanner(OpenInNerminalBannerAction::Close),
