...
use crate::terminal::view::inline_banner::{OpenInNerminalBannerAction, OpenInNerminalBannerState};
use crate::util::openable_file_type::{
    OpenableFileType, is_file_openable_in_warp, renders_in_warp_notebook_viewer,
};

#[cfg(test)]
#[path = "open_in_nerminal_tests.rs"]
mod tests;
...
impl TerminalView {
    pub(super) fn maybe_suggest_open_in_nerminal(
        &mut self,
        ...
        if let Some(active_block_metadata) = self.active_block_metadata.as_ref() {
            ...
            ctx.spawn(
                ...
                move |view, maybe_match, ctx| {
                    ...
                    {
                        view.suggest_open_in_nerminal(openable_path, session, ctx);
                    }
                },
            );
        }
    ...
    pub fn is_open_in_nerminal_banner_open(&self) -> bool {
        self.inline_banners_state.open_in_nerminal_banner.is_some()
    }

    fn close_open_in_nerminal_banner(&mut self, banner_id: usize) {
        self.model
            .lock()
            .block_list_mut()
    ...
    fn open_in_nerminal_banner_type_dismissed(
        &self,
        ...
        if renders_in_warp_notebook_viewer(&openable_path.path) {
            *general_settings.open_in_nerminal_banner_dismissed_for_markdown
        } else {
            *general_settings.open_in_nerminal_banner_dismissed_for_code_and_text
        }
    }

    /// Insert a suggestion banner for opening the file `openable_path`, originating from
    /// `session`, in a Warp pane.
    fn suggest_open_in_nerminal(
        &mut self,
        openable_path: OpenablePath,
        session: Arc<Session>,
        ctx: &mut ViewContext<Self>,
    ) {
        if self.open_in_nerminal_banner_type_dismissed(&openable_path, ctx) {
            return;
        }

        // We only show a banner for the most recent command.
        if let Some(prev_state) = &self.inline_banners_state.open_in_nerminal_banner {
            self.close_open_in_nerminal_banner(prev_state.id);
        }

        let banner_id = self.inline_banners_state.next_banner_id();
        self.inline_banners_state.open_in_nerminal_banner = Some(OpenInNerminalBannerState::new(
            banner_id,
            openable_path,
            session,
        ));
        self.model
            ...
            .append_inline_banner(InlineBannerItem::new(
                banner_id,
                InlineBannerType::OpenInNerminal,
            ));
        ctx.notify();
    }

    pub fn handle_open_in_nerminal_banner_action(
        &mut self,
        action: OpenInNerminalBannerAction,
        ctx: &mut ViewContext<Self>,
    ) {
        match action {
            OpenInNerminalBannerAction::OpenFile => {
                if let Some(banner_state) = self.inline_banners_state.open_in_nerminal_banner.take()
                {
                    ...
                    self.close_open_in_nerminal_banner(banner_state.id);
                    ctx.notify();
                }
            }
            OpenInNerminalBannerAction::LearnMore => {
                if let Some(banner_state) = &self.inline_banners_state.open_in_nerminal_banner {
            ...
            OpenInNerminalBannerAction::Close => {
                if let Some(banner_state) = self.inline_banners_state.open_in_nerminal_banner.take()
                {
                    self.close_open_in_nerminal_banner(banner_state.id);
                    if renders_in_warp_notebook_viewer(&banner_state.target.path) {
                        GeneralSettings::handle(ctx).update(ctx, |settings, ctx| {
                            report_if_error!(
                                settings
                                    .open_in_nerminal_banner_dismissed_for_markdown
                                    .set_value(true, ctx)
                            );
                        });
                    } else {
                        GeneralSettings::handle(ctx).update(ctx, |settings, ctx| {
                            report_if_error!(
                                settings
                                    .open_in_nerminal_banner_dismissed_for_code_and_text
                                    .set_value(true, ctx)
                            );
                        });
                    }
    ...
    pub fn open_in_nerminal_banner_accessibility_content(
        &self,
        action: OpenInNerminalBannerAction,
    ) -> ActionAccessibilityContent {
        match action {
            OpenInNerminalBannerAction::OpenFile => {
                match &self.inline_banners_state.open_in_nerminal_banner {
                    Some(banner_state) => {
