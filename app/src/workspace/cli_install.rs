...
fn create_symlink_with_admin(source: &Path, target: &Path) -> Result<()> {
    ...
    let script = format!(
        "do shell script \"ln -sf {escaped_source} {escaped_target}\" with prompt \"Nerminal needs administrator privileges to install the command in /usr/local/bin.\" with administrator privileges"
    );
...
fn remove_file_with_admin(target: &Path) -> Result<()> {
    ...
    let script = format!(
        "do shell script \"rm {escaped_target}\" with prompt \"Nerminal needs administrator privileges to uninstall the command from /usr/local/bin.\" with administrator privileges"
    );
...
pub fn install_warpctrl() -> Result<()> {
    ...
    if !warpctrl_source.exists() {
        return Err(anyhow!(
            "Cannot install the control CLI: bundled wrapper not found at {}",
            warpctrl_source.display()
        ));
    }

    install_symlink(&warpctrl_source, &warpctrl_path, "control CLI")
}
